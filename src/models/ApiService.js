...
class ApiService {
  ...
  runSensors(sensors, callback) {
    ...
    const requestData = sensors
      .filter(sensor => sensor.selected)
      .map(sensor => sensor.toJSON());
    axios
      .put(sensorApiUrl, requestData)
      .then(response => {
        if (response.status == 200) {
          const devicesJson = response.data;
          const sensors = devicesJson.map((deviceJson, index) => {
  ...
  runProcessor(processor, callback) {
    ...
    const api = this;
    axios
      .put(processorApiUrl, requestData)
      .then(response => {
        if (response.status == 200) {
          const processorJson = response.data;
  ...
  connectProcessor(processor, callback) {
    const handleWebworkerMessage = function(e) {
      if (e.data.action == "error") {
        ...
        processor.webworker.removeEventListener(
          "message",
  ...
  uploadAnnotations(annotations, id, callback) {
    ...
    console.log(requestData);
    axios
      .put(annotationApiUrl, requestData)
      .then(response => {
        if (response.status == 200) {
          callback("success");
        } else {
